...
| IPADDR | IP address connected by the current User |
...
| TYPE | Session Type<br>• **WTHR **: Worker thread<br>• **CTHR **: Control thread<br>• **LGWR **: Log write process<br>• **CKPT **: Checkpoint process<br>• **LARC **: Log archive<br>• **AGENT **: Sequence process<br>• **MTHR **: Monitoring process<br>• **DBWR **: Data block write process<br>• **LNW **: Log network write process |
| SQL_ID | SQL identifier of the SQL statement currently being executed |
| SQL_CHILD_NUMBER | Sub-number of the currently executing SQL statement |
| PREV_SQL_ID | ID of the most recently executed SQL statement |
| PREV_CHILD_NUMBER | Sub-number of the most recently executed SQL statement |
| SQL_ET | Elapsed time (in seconds) since the SQL statement was executed |
| LOGON_TIME | Time at which the session was created |
| STATE | Wait state of the thread<br>• INVALID: Not initialized<br>• NEW: Created<br>• IDLE: Ready to execute<br>• RUNNING: Executing<br>• WAITING: Waiting for an internal message<br>• RECV_WAITING: Waiting for a client message<br>• STOP_BY_MTHR: Stopped by MTHR<br>• DEAD: Terminated |
...
| MODULE | Name of the module set by the user in the program running on the client |
...
| ROW_WAIT_ROW_NO | Currently locked row |
| CONSUMER_GROUP | Name of the consumer group to which the current session belongs |
| CONSUMED_CPU_TIME | Cumulative CPU time used by the session |

{% hint style="info" %}
**Note**

The LOGON_TIME field displays time based on the database timezone. There may be a difference from the user's local system time (browser time).
{% endhint %}
{% endtab %}
...
| LOGON TIME | Time the session started |
...
| SCHEMA NAME | Name of the schema currently in use |
| SCHEMA NUMBER | ID of the schema currently in use |
...
| XACT START | Time the current transaction of the process started |
| STATE CHANGE | Time the state was last changed |
| BACKEND XID | Transaction ID assigned when a query starts |
| BACKEND XMIN | Minimum transaction ID related to the current query |
...
To support real-time updates on the monitoring screen, an auto refresh feature is provided. Click the ⚙️ icon in the upper left to set the refresh interval. 
