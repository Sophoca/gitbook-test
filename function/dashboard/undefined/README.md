# Feature Usage Guide / Dashboard / Database Exploration
...
The database discovery feature is available only under the Root account.
{% endhint %}
...
You can view a list of hosts on which a database can be installed. Each host's **Hostname, CPU, Memory, and OS information**is provided along with the listing. The installable hosts category is read-only and requires no additional follow-up action.
...
| --- | --- | --- | --- |
| **Spec Change** | When the configuration of a database has changed | Increase/decrease in node count | **[Spec Change]** Click the button to apply the detected changes. |
...
Any node for which OwlDB could not successfully retrieve configuration information during the discovery process is marked as an abnormal node.

Abnormal nodes occur in the following two cases.

| Case | Description |
| --- | --- |
| When the DB configuration cannot be determined at all | The affected node is marked individually as an abnormal node. |
| When the DB configuration is only partially determined | The identified nodes are grouped together and displayed as a single abnormal node. |
